...
ItemPQ  dequeuePQ(PQ q) {
	...
	//shift to next and make that node to be a new head
	q->head = q->head->next;
	free(toDelete);
	return it;
}
...
void  updatePQ(PQ q, ItemPQ it) {
	...
	Node curr = q->head;
	Node toUpdate = NULL;
	//find node which contain item 'it'
	while (curr != NULL) {
...
static void reorder(PQ q, ItemPQ it) {
	...
	Node toUpdate = NULL;
	Node prev = NULL;
	//if it is the head change head, otherwise connect prev and next of it's node
	if ((curr->item).key == it.key) {
	    toUpdate = curr;
	    q->head = q->head->next;
	} else {
