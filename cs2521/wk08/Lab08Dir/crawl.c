...
int main(int argc, char **argv)
{
	...
	Graph URLg = newGraph(maxURLs);
	Set visited = newSet();
	while (!emptyQueue(todo) && nVertices(URLg) < maxURLs) {
		...
		while(!url_feof(handle)) {
			...
			while ((pos = GetNextURL(buffer, next, result, pos)) > 0) {
				printf("Found: '%s'\n",result);
				if ((nVertices(URLg) < maxURLs) ||
				     isConnected(URLg, next, result)) {
					addEdge(URLg, next, result);
				}
				if (!isElem(visited, result)) {
					insertInto(visited, result);
