...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
   ...
   int cnt = 0;
   if (g->connections[start]->type == BOAT && g->connections[end]->type == BOAT) {
       type[cnt] = BOAT;
       cnt++;
   }
   while (n != NULL) {
       if (n->v == end && n->type != BOAT) { //makes sure not to check for boat twice
           type[cnt] = n->type;
           cnt++;
       }
       n = n->next;
   }
   return cnt;  // to keep the compiler happy
}
